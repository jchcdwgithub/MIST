...
### Export Ekahau APs
This task reads AP names and models from an Ekahau file or folder and writes xlsx spreadsheets with columns `AP Name`, `Model`, and `Serial` (empty, for manual entry). Exported AP models are normalized to the `AP-` + digits pattern (plus any characters attached before a space); text after the first space is removed (for example `AP-765 Omni` becomes `AP-765`). Set `esx_file` for one file or `esx_folder` to process every `.esx` in a directory (one `{filename}_aps.xlsx` per file).
